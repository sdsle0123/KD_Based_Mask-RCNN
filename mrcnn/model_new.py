...
def resnet_graph_student(input_image):

    # from tensorflow.keras.applications.resnet_v2 import ResNet50V2
    # # 加载 ResNet50 模型，并将新的输入层作为输入
    # student = ResNet50V2(input_tensor=input_image, include_top=False)
    # SC2 = student.get_layer('conv2_block3_preact_relu').output
    # SC3 = student.get_layer('conv3_block4_preact_relu').output
    # SC4 = student.get_layer('conv4_block6_preact_relu').output
    # SC5 = student.get_layer('post_relu').output
    # student_new = KM.Model(inputs=student.input, outputs=[SC2, SC3, SC4, SC5])
    # student_new.load_weights(r"D:\Coding\MRCNN_KD\student_model_weights_0427.h5")
    # student_new.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    # student.trainable = True

    student_new = keras.models.load_model( r"D:\Coding\MRCNN_KD\student_model0504.h5",
    custom_objects={'BatchNorm':KL.BatchNormalization})
    student_new.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    student_new.trainable = True

    r = student_new(input_image)
